...
## Contract changes drive the implementation remotely

Conformance decays if it depends on a human noticing contract changes. On every merge to `main` touching `specs/`, `dispatch-contract-change.yml` sends a `repository_dispatch` to the implementation repo, whose `contract-sync` workflow (installed by the skill's phase 5) runs an agent that updates the implementation and opens a PR gated by the implementation's own definition-of-done CI.

Regenerate-from-scratch was rejected: it discards implementation history, makes every contract tweak a full rewrite review, and cannot converge. Instead the loop is convergent by construction — the implementation pins the contract sha it implements in a `CONTRACT_REF` file (dispatch for the same sha is a no-op); the agent scopes its work from the *diff* between the implemented and target contract refs, reusing the same oasdiff/asyncapi-diff tooling as `task check:compat`; and the branch name is deterministic per contract sha, so re-runs update one PR rather than piling up new ones. Trigger granularity is every merged contract change (not tags) — matching how the contracts actually evolve here; batching via release tags remains available later by pointing the dispatch at tag pushes.

The gate stays honest in two ways: the sync agent is instructed to open a draft PR reporting failures rather than weaken tests or specs to force green, and agent PRs are created via the Claude GitHub App rather than the default `GITHUB_TOKEN` — PRs from `GITHUB_TOKEN` never trigger workflows, which would silently remove the CI gate the whole loop depends on.
